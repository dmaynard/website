...
## How To Add a New Category to the Visuaizations
...
Since there are only 50 States, and therefore only 50 data points for static State data, I find it is usuallty easier to manually add the new data points to an existing data file than to create a new file.  This also saves having to add code to read and parse the file.  A good file to use is  StateAbbreviations.json. This file contains several data items for each of the 50 states.  This file is a "File Attachment" used by this Observable Notebook.  You can  download a copy of the file to your local machine so you can edit it by going to the File Attachments submenu under the "..." button.
...
stateGlobals = statePopulations.data.map(e => ({
  ...
  stateGlobals gathers together all the static classification data for the states from various data file attachments.  The code above adds a new field, "pctVaccinated20210531", to each State Object and sets its value to the appropriate PctVaccinatedBucketName based upon the state's ranking (0-49) % vaccinated by finding that state's index in the sorted file stateAbbreviationsByPctVaccinated. 
  The mapping from index to name is controlled by the "group by" radio button above, which is in turn controlled by the followinhg json constant cell 
  ```
  rankingCategories = [
  [
